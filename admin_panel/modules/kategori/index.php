...
$sort_by = $_GET['sort'] ?? 'kategori_id';
$sort_order = $_GET['order'] ?? 'ASC';

$allowed_sort_columns = ['kategori_id', 'nama_kategori', 'created_at'];
if (!in_array($sort_by, $allowed_sort_columns)) {
    $sort_by = 'kategori_id';
}
$sort_order = strtoupper($sort_order) === 'DESC' ? 'DESC' : 'ASC';

$base_sql_select = "SELECT kategori_id, nama_kategori, deskripsi, created_at FROM kategoriproduk";
$base_sql_count = "SELECT COUNT(*) as count FROM kategoriproduk";
$where_clauses = [];
$params = [];
$types = "";
...
$kategoris = get_data_list($conn, $sql_select_final, $params, $types, $items_per_page, $offset);
...
<head>
    ...
    <title>Manajemen Kategori Produk - Bloomarie Admin</title>
    ...
    <div class="main-content">
        ...
        <div class="data-table-card">
            <div class="card-header">
                ...
                <a href="tambah_kategori.php" class="btn btn-success btn-sm"><i class="fas fa-plus me-1"></i> Tambah Kategori</a>
            </div>
            <div class="card-body">
                <?php if (isset($_SESSION['kategori_success_message'])): ?>
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        <?php echo sanitize_output($_SESSION['kategori_success_message']); unset($_SESSION['kategori_success_message']); ?>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                <?php endif; ?>
                <?php if (isset($_SESSION['kategori_error_message'])): ?>
                    <div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <?php echo sanitize_output($_SESSION['kategori_error_message']); unset($_SESSION['kategori_error_message']); ?>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                <?php endif; ?>
                ...
                <form method="GET" class="mb-4">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari nama atau deskripsi kategori..." value="<?php echo sanitize_output($search_query); ?>">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th><a href="?search=<?php echo urlencode($search_query); ?>&sort=nama_kategori&order=<?php echo ($sort_by == 'nama_kategori' && $sort_order == 'ASC' ? 'DESC' : 'ASC'); ?>">Nama Kategori <?php if($sort_by == 'nama_kategori') echo $sort_order == 'ASC' ? '<i class="fas fa-sort-up"></i>' : '<i class="fas fa-sort-down"></i>'; ?></a></th>
                                <th>Deskripsi</th>
                                <th><a href="?search=<?php echo urlencode($search_query); ?>&sort=created_at&order=<?php echo ($sort_by == 'created_at' && $sort_order == 'ASC' ? 'DESC' : 'ASC'); ?>">Dibuat Pada <?php if($sort_by == 'created_at') echo $sort_order == 'ASC' ? '<i class="fas fa-sort-up"></i>' : '<i class="fas fa-sort-down"></i>'; ?></a></th>
                        ...
                        <tbody>
                            <?php if (count($kategoris) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo sanitize_output($kategori['nama_kategori']); ?></td>
                                    <td><?php echo nl2br(sanitize_output(substr($kategori['deskripsi'] ?? '', 0, 100))) . (strlen($kategori['deskripsi'] ?? '') > 100 ? '...' : ''); ?></td>
                                    <td><?php echo date('d M Y, H:i', strtotime($kategori['created_at'])); ?></td>
                                    <td class="text-center action-buttons">
                                        <a href="edit_kategori.php?kategori_id=<?php echo $kategori['kategori_id']; ?>" class="btn btn-sm btn-primary" title="Edit"><i class="fas fa-edit"></i></a>
                                        <a href="proses_kategori.php?action=delete&kategori_id=<?php echo $kategori['kategori_id']; ?>"
                                           class="btn btn-sm btn-danger" title="Hapus"
                                           onclick="return confirm('Apakah Anda yakin ingin menghapus kategori \'<?php echo htmlspecialchars(addslashes($kategori['nama_kategori']), ENT_QUOTES); ?>\'? Ini bisa mempengaruhi produk terkait.');">
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="text-center py-4 text-muted fst-italic">
                                    <?php if (!empty($search_query)): ?>
                                        Tidak ada kategori ditemukan untuk pencarian "<?php echo sanitize_output($search_query); ?>".
                                    <?php else: ?>
                ...
                <nav aria-label="Paginasi Kategori" class="mt-4 d-flex justify-content-center">
                    <ul class="pagination pagination-sm">
                      ...
                      admin_panel/modules/kategori/index.php          <a class="page-link" href="?search=<?php echo urlencode($search_query); ?>&sort=<?php echo $sort_by; ?>&order=<?php echo $sort_order; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                            </li>
